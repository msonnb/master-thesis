...
\title{\vspace{-2.8cm}\textsc{\large Research Proposal} \\ ActivityPub
  compatibility
for Bluesky}
...
compatible with ActivityPub and thus all Mastodon instances. We
therefore decsribe an ideal compatibility state that outlines how
...
how close the ideal compatibility state can be reached with
...
subscribe, and moderate interactions accross both the ATProto and
...
The first work package is about making Bluesky users and content
available on Mastodon. It forms itself around the following user story:

\begin{quote}
  \itshape
  \enquote{As a Bluesky user, I can give my handle to a Mastodon user. They
...
Bluesky user discoverable and viewable on Mastodon. In order to
...
\begin{enumerate}
  \item \textbf{Identity \& discovery:} WebFinger endpoint that maps
    ...
    allows discovery of the user on Mastodon (i.e. appearing in
  ...
  \item \textbf{Outbound broadcast:} When a Bluesky post is created
    ...
    Replies by users on the same PDS can also be delivered via
    ActivityPub; replies from users on other PDSs will not be visible
    in Mastodon at this point. Additionally, all users posts can be
    fetched from ActivityPub using a user's \texttt{outbox} endpoint.
  \item \textbf{Inbound listening:} Accept \emph{Follow}, \emph{Like},
    and \emph{Create(Note)} (reply) activites. Follows will be stored
    and push notifications will be created for likes and replies.
\end{enumerate}
...
\begin{quote}
  \itshape
  \enquote{As a Bluesky user, I can search for a Mastodon user inside my
...
challenge is tha one cannot write external data into the user's
...
the PDS that stores all incoming ActivityPub activites. The following
steps outline the features of this work package:

\begin{enumerate}
  \item \textbf{Virtual users:} When a user searches for
    \texttt{@user@mastodon.social}, the PDS must fetch that actor's
    profile and create a temporary \enquote{shadow profile} in the
    local database such that the Bluesky client can render the
    profile data in the UI. The Bluesky user can follow that profile,
  ...
  \item \textbf{Shared inbox:} To scale efficiently, we implement a shared
    ...
    include further information about routing activites to specific users.
...
\begin{enumerate}
  \item \textbf{Moderation federation:} If a user blocks an account
    ...
    ATProto's moderation system. Similary, content moderation (labels
  ...
  \item \textbf{Account migration:} Implementing the ActivityPub
    \textit{Move} activity, allowing a user to forward their
    followers to a different Mastodon instance if they choose to leave our PDS.
\end{enumerate}
