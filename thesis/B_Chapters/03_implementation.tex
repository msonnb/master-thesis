...
not provided. Only three variables are required, the PDS URL, PDS
hostname, and PDS admin token. All other settings have sensible
...
otherwise, it constructs an HTTPS URL, reflecting the assumption that
...
be overridden with a file path for production use.
...
adds newly created local posts to the \ttt{ap\_monitored\_post} table
...
\subsection{Bridge Account System}\label{sec:bridge-account-system}

\subsubsection{Why Two Bridge Accounts?}
...
\subsubsection{Account Lifecycle}
...
Since both bridge accounts post content on behalf of users and do not
...
instances to link the ActivityPub actor back to the original Bluesky
user and profile.
...
After conversion completes, Fedisky stores the post mapping in the
...
performs an additional step: It looks up the bridge post mapping to
...
\begin{itemize}
  ...
  \item \ttt{GET /users/\{did\}/outbox} \\
    ...
    This allows Mastodon instances to fetch the user's content and
...
content is parsed into plain text using the \ttt{html-to-text} NPM
...
generating engagement notifications for posts that are not managed by
